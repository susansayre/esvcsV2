...
varList = {'optTempPay','UB','rpf','probConserve','expRegPay2','expProbConserve2' 'noOfferUB','noOfferProbConserve','noOfferExpRegPay2','noOfferRegPay'};
...
for ii=1:numel(output)
	...
	for jj=1:numel(varList)
		eval(['infoDelta{ii}.' varList{jj} '=reshape(compareOutResults{ii}.' varList{jj} '(:,2:end) - repmat(compareOutResults{ii}.' varList{jj} '(:,1),1,outputShapes{ii}(2)),outputShapes{ii});'])
		eval(['infoPercDelta{ii}.' varList{jj} '=infoDelta{ii}.' varList{jj} './reshape(repmat(compareOutResults{ii}.' varList{jj} '(:,1),1,outputShapes{ii}(2)),outputShapes{ii});'])
	end
end