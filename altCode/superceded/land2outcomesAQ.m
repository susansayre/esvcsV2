function expOut = land2outcomesAQ(outputVarName,privValVec,UBVec,P,useReal)
	...
	expOut = integral(@(sigVal)p2Out(sigVal,privValVec,UBVec,P,outputVarName),-Inf,Inf,'ArrayValued',1,'AbsTol',1e-4,'RelTol',1e-2);

	function p2OutVal = p2Out(signalVal,privateVals,assumeUBs,P,outVarName)
...
% 			Phat = P; Phat.prevP2.optOffers = optimalOffers;
			optimalOffers = optOfferSimple(condRegInfo,P);

		optimalOffersFull(:,1) = optimalOffers(CInd);
		
		[land2Val,land2Choice] = max([optimalOffersFull privateVals(:) ],[],2);
		...
		switch outVarName
			...
			case 'reg2Pay'
				condMeanEnv = P.meanEnv + P.sig.re/P.sig.rp*P.rho.re_rp/(1-P.rho.se_rp^2)*(privateVals-P.pubVal-P.meanPriv) + signalVal*(1-P.rho.se_rp*P.rho.re_rp/(sqrt(P.sigShr)*(1-P.rho.se_rp^2)));
